...
export default function ThemeShowcase() {
  ...
  const handleMigrate = async () => {
    try {
      ...
      // Upload each member with their userCode as the document ID
      for (const member of mockMembers) {
        await setDoc(doc(membersRef, member.userCode), member);
      }
      
      setMigrationStatus('Success! 16 members added to Firestore.');
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="app-container">
      ...
      <section style={{ marginBottom: '24px', padding: '20px', background: '#FFFBEB', borderRadius: '8px' }}>
        <h2 style={{ marginBottom: '10px', color: '#D97706' }}>Database Migration Tool</h2>
        <p style={{ marginBottom: '10px' }}>Click the button below to upload the local `mockMembers` directly into your connected Firestore database.</p>
        <button className="btn btn-primary" onClick={handleMigrate}>
          Push Data to Firestore
        </button>
        {migrationStatus && <p style={{ marginTop: '10px', fontWeight: 600 }}>{migrationStatus}</p>}
      </section>
