...
import { Camera, UploadCloud, Loader2, FileText, CheckCircle2, AlertTriangle, Save, Trash2, X, Plus, ImagePlus, RefreshCw } from 'lucide-react';
import { extractSingleImageData } from '../services/ocrService';
...
export default function OCRTab() {
  ...
  const handleScan = async () => {
    ...
    if (!extractedData) setExtractedData([]);

    let hasError = false;
    let localExtractedData = extractedData ? [...extractedData] : [];
    let totalExtractedThisRun = 0;
    let accumulatedNewData = [];

    for (let i = 0; i < unprocessedImages.length; i++) {
      const img = unprocessedImages[i];
      const imgGlobalIndex = images.findIndex(x => x.name === img.name && x.size === img.size);
      
      setScanningImageIndex(imgGlobalIndex);
      setScanProgress({ current: i + 1, total: unprocessedImages.length });
      
      try {
        const data = await extractSingleImageData(img.base64Data);
        
        const startIndex = localExtractedData.length + accumulatedNewData.length;
        ...
        accumulatedNewData.push(...mappedData);
        totalExtractedThisRun += mappedData.length;
        
        // mark this specific image as processed
        setImages(prev => prev.map((im, idx) => idx === imgGlobalIndex ? { ...im, processed: true } : im));
        
      } catch (error) {
        console.error('Scan Error on image', i + 1, error);
        setScanError(`Failed on image ${i + 1}: ${error.message || 'Unknown error'}`);
        hasError = true;
        break;
      }
    }

    if (accumulatedNewData.length > 0) {
      // Strip out auto-generated gaps so we can recalculate from a clean slate
      const previousCleanData = localExtractedData.filter(row => !row.isAutoGenerated);
      const combined = [...previousCleanData, ...accumulatedNewData];
    ...
    if (!hasError && totalExtractedThisRun > 0) {
      showToast(`Successfully extracted ${totalExtractedThisRun} new entries.`, 'success');
    }
  };
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: '32px' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '32px' }}>
          ...
          <div className="card">
                  ...
                  <div className="image-grid">
                    {images.map((img, i) => (
                      <div key={i} className="preview-container">
                        <img src={img.preview} alt={`Register Preview ${i + 1}`} style={{ opacity: img.processed ? 0.6 : 1 }} />
                        
                        {scanningImageIndex === i && (
                        ...
                        {img.processed && (
                          <button
                            className="rescan-image-btn"
                            onClick={(e) => {
                              e.stopPropagation();
                              setImages(prev => prev.map((im, idx) => idx === i ? { ...im, processed: false } : im));
                            }}
                            title="Rescan image"
                            disabled={isProcessing}
                            style={{ 
            ...
            {isProcessing && (
              <div style={{ marginTop: '20px', padding: '16px', background: 'var(--bg-muted)', borderRadius: '12px', border: '1px solid var(--border-default)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', marginBottom: '16px' }}>
                  <Loader2 className="spinner" size={20} color="var(--primary)" />
                  <span style={{ fontSize: '15px', fontWeight: 600, color: 'var(--text-primary)' }}>
                    {scanProgress.current > 0 ? `Scanning image ${scanProgress.current} of ${scanProgress.total}...` : 'Initializing...'}
                  </span>
                </div>
                <div className="progress-bar-container" style={{ width: '100%', background: 'var(--bg-card)', borderRadius: '8px', padding: '4px', border: '1px solid var(--border-default)' }}>
                  <div 
                    ...
                    style={{ 
                      ...
                      borderRadius: '4px', 
                      width: `${scanProgress.total > 0 ? ((scanProgress.current - 1) / scanProgress.total) * 100 : 0}%`,
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>
                      ...
                      <th style={{ padding: '16px', textAlign: 'center', fontSize: '13px', color: 'var(--text-tertiary)', fontWeight: 600, borderBottom: '1px solid var(--border-default)', textTransform: 'uppercase' }}>Hours Consumed</th>
                    ...
                    {extractedData.map((row, index) => {
                      ...
                      return (
                        <tr key={row.id} style={{ borderBottom: '1px solid var(--border-default)', background: hasReadingError ? 'var(--danger-light)' : row.isAutoGenerated ? 'var(--warning-light)' : 'transparent' }}>
                          ...
                          <td style={{ padding: '12px 16px', fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)', textAlign: 'center', fontFamily: 'Outfit' }}>
                            {(!isNaN(start) && !isNaN(end) && !hasReadingError) ? (end - start).toString() : '-'}
                          </td>
                          <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                      ...
                      <div className="mobile-entry-header" style={{ borderBottom: 'none', paddingBottom: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                          <div className="mobile-entry-hours">
                            <span className="mobile-entry-hours-value">{(!isNaN(start) && !isNaN(end) && !hasReadingError) ? (end - start).toString() : '-'}</span>
                            <span className="mobile-entry-hours-label">units</span>
                          </div>
