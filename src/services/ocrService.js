/**
 * Extracts data from a single base64 image string using the secure Vercel backend.
 ...
 */
export const extractSingleImageData = async (base64Image) => {
  try {
...
The image contains MULTIPLE rows of data. You must extract every single row you can read across the provided image.
...
[
  {
    ...
    console.log(`Processing image with Gemini API directly...`);
    ...
    throw lastError || new Error(`All OCR models failed for this image.`);

  } catch (error) {
    console.error('Error in extractSingleImageData:', error);
    throw error;
  }
};
